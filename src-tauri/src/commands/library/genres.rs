...
pub async fn delete_genre(id: i64, pool: tauri::State<'_, SqlitePool>) -> Result<(), String> {
    ...
    sqlx::query("DELETE FROM film_genres WHERE genre_id = ?").bind(id).execute(pool.inner()).await.map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM person_genres WHERE genre_id = ?").bind(id).execute(pool.inner()).await.map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM wiki_entries WHERE entity_type = 'genre' AND entity_id = ?").bind(id).execute(pool.inner()).await.map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM genres WHERE id = ?").bind(id).execute(pool.inner()).await.map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn link_film_genre(film_id: i64, genre_id: i64, pool: tauri::State<'_, SqlitePool>) -> Result<(), String> {
    sqlx::query("INSERT OR IGNORE INTO film_genres (film_id, genre_id) VALUES (?, ?)")
        .bind(film_id).bind(genre_id).execute(pool.inner()).await
        .map(|_| ()).map_err(|e| e.to_string())
}
...
pub async fn link_person_genre(person_id: i64, genre_id: i64, pool: tauri::State<'_, SqlitePool>) -> Result<(), String> {
    sqlx::query("INSERT OR IGNORE INTO person_genres (person_id, genre_id) VALUES (?, ?)")
        .bind(person_id).bind(genre_id).execute(pool.inner()).await
        .map(|_| ()).map_err(|e| e.to_string())
}
