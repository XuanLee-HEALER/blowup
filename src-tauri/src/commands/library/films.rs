use super::{
    FilmDetail, FilmFilterResult, FilmListEntry, FilmPersonEntry, FilmSummary, GenreSummary,
    ReviewEntry, TmdbMovieInput,
};
...
pub async fn add_film_from_tmdb(
    ...
    if let Some(existing_id) =
        sqlx::query_scalar::<_, i64>("SELECT id FROM films WHERE tmdb_id = ?")
            .bind(tmdb_movie.tmdb_id)
    ...
    for person in &tmdb_movie.people {
        let person_id: i64 = if let Some(tmdb_id) = person.tmdb_id {
            if let Some(existing) =
                sqlx::query_scalar::<_, i64>("SELECT id FROM people WHERE tmdb_id = ?")
                    .bind(tmdb_id)
                    .fetch_optional(pool.inner())
                    .await
                    .map_err(|e| e.to_string())?
            ...
            if let Some(existing) =
                sqlx::query_scalar::<_, i64>("SELECT id FROM people WHERE name = ?")
                    .bind(&person.name)
                    .fetch_optional(pool.inner())
                    .await
                    .map_err(|e| e.to_string())?
...
pub async fn delete_film(id: i64, pool: tauri::State<'_, SqlitePool>) -> Result<(), String> {
    sqlx::query("DELETE FROM film_genres WHERE film_id = ?")
        ...
        .await
        .map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM person_films WHERE film_id = ?")
        .bind(id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
    sqlx::query("DELETE FROM reviews WHERE film_id = ?")
        .bind(id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
    sqlx::query("UPDATE library_items SET film_id = NULL WHERE film_id = ?")
        .bind(id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
...
pub(crate) async fn list_films_filtered_inner(
    ...
    // Count query
    let mut count_qb =
        QueryBuilder::<sqlx::Sqlite>::new("SELECT COUNT(DISTINCT f.id) FROM films f");
    if genre_id.is_some() {
    ...
    qb.push(" WHERE 1=1");
    apply_film_filters(
        &mut qb, &query, genre_id, year_from, year_to, min_rating, has_file,
    );
    ...
    let dir = if sort_desc.unwrap_or(true) {
        "DESC"
    } else {
        "ASC"
    };
...
mod tests {
    ...
    async fn test_list_films_filtered_has_file() {
        let pool = setup().await;
        sqlx::query("INSERT INTO films (title) VALUES (?)")
            .bind("Film A")
            .execute(&pool)
            .await
            .unwrap();
